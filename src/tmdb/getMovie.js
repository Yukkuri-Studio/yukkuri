...
module.exports = {
  ...
  async getTvShow (data, i) {
    const res = data.results.slice(0, 10)

    if (!res.length) {
      return i.channel.send({ content: 'Sorry, There is no result for movie that you want to search' })
    }

    const select = []
    for (const data of res) {
      select.push({ label: `${data.name}`, value: data.name, description: `Information about "${data.name}"` })
    }

    const menu = new MessageSelectMenu()
      .setCustomId('tvshow-select')
      .addOptions(select)

    const row = new MessageActionRow().addComponents([menu])

    const msg = await i.channel.send({ content: 'Choice the movie from menu bellow', components: [row] })
    Interaction.awaitTvInteraction(msg, data, i.member)
  }
}
